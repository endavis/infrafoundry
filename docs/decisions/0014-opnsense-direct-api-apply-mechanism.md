...
**Amended:** 2026-06-18 (#789) — `cron_jobs` per-component decision recorded (stock direct-REST against `cron/settings/*` MVC controller; list-shape full-reconcile; description-suffix identity scheme `[infrafoundry:<name>]`; new `cron_reconfigure` finalization hook — fourth after `kea_reconfigure`, `unbound_reconfigure`, and `radvd_reconfigure`; destroy calls `reconfigure` directly; `plugin_conflicts()` guards against shared-command collisions; `lock: true` per-resource skips diff and destroy paths).
**Status:** Accepted
...
### `cron_jobs` (#789, 2026-06-18)

**Mechanism:** Stock direct-REST against the OPNsense 26.x `cron/settings/*` MVC controller — scheduled tasks (cron jobs). List-shape, full-reconcile semantics. Each YAML entry is a distinct cron job; managed jobs absent from YAML are deleted; unmanaged jobs (no identity suffix) are never touched.

- **Endpoints:** `POST cron/settings/searchJobs` (paginated `{"rows": [...]}`), `POST cron/settings/addJob` (create), `POST cron/settings/setJob/<uuid>` (update), `POST cron/settings/delJob/<uuid>` (delete), `POST cron/service/reconfigure` (apply pending).
- **Identity:** description-suffix scheme — `<user description> [infrafoundry:<name>]` appended to the OPNsense `description` field at apply time (or bare `[infrafoundry:<name>]` when no operator description is supplied). Unmanaged jobs without the suffix are invisible to the diff engine. `parse_identity()` raises `OpnsenseDriftError` if a description starts with `[infrafoundry:` but does not match the full pattern — prevents silent identity corruption.
- **Wire field mapping (confirmed via live probe 2026-06-18):** schedule fields pass through 1:1 — the OPNsense MVC model uses `days`/`months`/`weekdays`, which are identical to the YAML keys. No translation occurs in `to_payload()` or `_row_to_live()`. `enabled` serializes as `"1"`/`"0"` strings. `origin` is a REQUIRED wire field; InfraFoundry always forces `"origin": "cron"` on every write so that `delJob` can delete the record (OPNsense's `delJob` is delete-protected for non-`"cron"` origins — e.g., `"AcmeClient"` jobs are only deletable through their own plugin UI). `origin` is excluded from drift comparison in `_needs_update()` to prevent spurious updates. `export_to_yaml()` always emits `origin: cron`. Validator emits a WARNING (not an error) when the operator sets a non-`"cron"` origin in YAML, explaining that the value will be overridden at apply time.
- **Schedule grammar:** `minutes`, `hours`, `days`, `months`, `weekdays` each accept `*`, `*/N`, plain integer, `L-H`, `L-H/N`, or comma-separated lists. Range boundaries are validated at plan time by `CronJobsValidator` against per-field bounds: minutes 0–59, hours 0–23, days 1–31, months 1–12, weekdays 0–7.
- **Apply semantics (per #789):** Full reconcile. Adds → updates → deletes (service processes in that order via `apply_diff`). `add_only=True` suppresses updates and deletes. `lock: true` per-resource is honored in both apply and destroy paths — the locked job is counted under `locked_skipped` and never touched. `plugin_conflicts()` detects unmanaged live jobs that share a `command` with a managed YAML job and emits a WARNING (not an error — both will execute the same binary; operator is informed but not blocked).
- **Reconfigure semantics:** Manager declares `FINALIZATION_HOOK = "cron_reconfigure"`. The runner fires `OPNsenseProvider._reconfigure_cron(env_name)` exactly once per apply when any cron job changed state, calling `CronJobsService.reconfigure()` which POSTs `cron/service/reconfigure`. Fourth hook key after `kea_reconfigure`, `unbound_reconfigure`, and `radvd_reconfigure`. **Destroy calls `reconfigure` directly** — unlike apply, the `cron_reconfigure` finalization hook is not fired by the runner for destroy paths, so `CronJobsManager.destroy()` calls `service.reconfigure()` inline after any deletions (and skips it when nothing was deleted).
- **Validator scope:** `CronJobsValidator` (pure-check, no live-box access). Validates: unique names within the resource list; `description` must not contain `[infrafoundry:...]` (reserved); `command` non-empty; `who` non-empty; schedule fields per-field range/grammar; non-`"cron"` `origin` values emit a WARNING (`passed=True`, `level=WARNING`) informing the operator that their value will be overridden on the wire. Check names are structured as `cron_jobs_unique_names`, `cron_job_<name>_description_reserved`, `cron_job_<name>_command_empty`, `cron_job_<name>_who_empty`, `cron_job_<name>_<field>_range`, `cron_job_<name>_origin_override`.
- **Rollback strategy:** Per-job write failures surface in the OPNsense response and raise on the API layer, failing the apply loud. No transactional rollback — successful per-job writes that landed before a later failure stay applied. `cron/service/reconfigure` runs once per apply via the finalization hook.
- **Cross-reference notes:** None. Self-contained list resource; no cross-references to other resource types.
...
A fourth hook, `cron_reconfigure` (#789), is declared on `CronJobsManager`. The cron controller's reconfigure verb is `cron/service/reconfigure`; like `radvd_reconfigure`, there is only one declarer today. **Note:** the `cron_reconfigure` hook only fires on the `apply` path — `destroy` in `CronJobsManager` calls `service.reconfigure()` directly after deletions because the runner's finalization-hook mechanism is not triggered for destroy operations.
