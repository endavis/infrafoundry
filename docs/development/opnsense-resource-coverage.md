...
| `<OPNsense>/Firewall/Alias` | `aliases` | Supported (terraform write path; config-migrate extractor added in #747). YAML schema covers the base fields (`name`, `type`, `description`, `content`, `enabled`) plus type-specific `proto` (geoip), `updatefreq` (urltable / urltable_ports), `categories`, `counters`, and `interface` (dynipv6host); these optional fields are emitted only when the live record carries a non-default value, so existing operator YAML round-trips unchanged. The terraform template renames `proto` → `ip_protocol` and `updatefreq` → `update_freq` to match the `browningluke/opnsense_firewall_alias` provider schema (#765); the YAML field names are unchanged so existing migrated configs continue to work. **`counters`** is preserved in the YAML round-trip (extractor reads it from the live API and emits it to YAML when set) but is intentionally **not rendered** to terraform — the `browningluke` provider has no settable equivalent (the `stats` schema attribute is computed/read-only) (#765). System-internal aliases (`type: internal` per-interface auto-generated entries like `__lan_network`, and `type: external` system-managed tables like `bogons` / `sshlockout`) are silently filtered from extractor output — OPNsense regenerates them server-side and they're not operator-writable via the alias controller; including them would produce a YAML block the terraform write path would attempt to (re)create on apply. |
