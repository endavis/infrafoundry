...
- **Must maintain mapping** between protocols and command strings
...
- **Semantic ambiguity**: auto_approve means different things per runner
...
- **Verbose**: Requires check at every call site
...
- **Breaking change**: Existing code without checks will break
...
- **String duplication**: Message format repeated at call sites
...
- **Requires mypy 0.971+**: Check CI/CD config
- **Occasional cast needed**: List comprehensions, complex flows
...
- **Migration needed**: Update all return statements
- **Runtime unchanged**: TypedDict is just hints, no runtime checks
...
- **Maintenance**: Need to update when adding new protocols
...
- **Document breaking changes**: Clear CHANGELOG and upgrade guide
- **Semantic versioning**: Bump to v2.0 when removing run()
...
- **Undocumented**: No official plugin API yet
- **Unsupported**: Breaking changes can still happen
...
- **Not protocol-specific**: Can't have different order for destroy vs apply
...
- **Needs testing**: Pulumi runner may be untested in CI
- **Documentation**: Need to document Pulumi support
