...
### Breaking Changes
- **BREAKING: `config migrate` component names renamed to Python-identifier form (#726)**. The two pre-#726 OPNsense components — `kea/dhcp` and `isc-to-kea` — are now `kea_dhcp` and `isc_to_kea`, matching the new extractor-registry key. There is no transparent alias; the hyphenated/slash forms are no longer accepted. Migration: update any scripts or runbooks that pass `--component kea/dhcp` to `--component kea_dhcp`, and `--component isc-to-kea` to `--component isc_to_kea`. The default output filename also follows the new name (`migrated-isc_to_kea.yaml`). The same change introduces a pluggable extractor registry that makes 8 additional OPNsense components reachable via `config migrate` (`vlans`, `interface_assignments`, `nat_rules`, `gateways`, `static_routes`, `virtual_ips`, `unbound_host_alias`, `unbound_forward`). The pre-#726 `OPNsenseProvider.migrate_<resource>` provider methods are retained as deprecated shims for one minor version. See [ADR-0014 §8](docs/decisions/0014-opnsense-direct-api-apply-mechanism.md).
